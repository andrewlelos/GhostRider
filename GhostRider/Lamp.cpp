...
Lamp::Lamp(unsigned int programID, float pos[3]) {
	this->PID = programID;
	...
	std::string objectPath = "external_files/Lamp/Lamp.obj";
...
/*	Create a VAO and bind it to the shader program
	that is PID.
 */
void Lamp::createVAO(unsigned int PID) {
	glUseProgram(PID);
...
void Lamp::render(unsigned int PID) {
	glUseProgram(PID);
...
void Lamp::loadRGBTexture(const char *path) {
	int x, y, n;
